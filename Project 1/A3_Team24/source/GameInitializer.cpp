		...
		if (choice == 1) {
			...
			while (true) {
				...
				if (computerChoice == 1) {
					...
					p2 = new Player("Greedy Player", 14, 0, t1, c1, b2, "white");
					break;
				}
					...
					p2 = new Player("Moderate Player", 14, 0, t1, c1, b2, "black");
					break;
				}
				else {
			...
			Player *	p1 = new Player("Greedy Player", 14, 0, t1, c1, b1, "white");


			int ran2 = rand() % 15;
			cout << ran2 << endl;
			b2 = new Bidding("Moderate Player", ran2);
			Player *	p2 = new Player("Moderate Player", 14, 0, t1, c1, b2, "black");
			system("cls");
			system("pauser");
...
void GameInitializer::playGame(int playerNumber, Hand* hand, Deck* deck, GameInitializer* gm) {
	...
	State* s1 = new State(hand,players[playerNumber], this->getCurrentMap());//Phase object
...
void GameInitializer::actions(Player* p, string action) {
	...
	if (action == "Move2") {
		...
		if (p->GetName() == "Greedy Player" || p->GetName() == "Moderate Player") {
			...
			p->BuildCity(allTerritories[temp[temp2]]);//biuld the city if the name of territories is yours
			cout << "You get a token in your territory" << endl;
			allTerritories[temp[temp2]]->SetTokenName("token");
			allTerritories[temp[temp2]]->SetIsToken(true);
